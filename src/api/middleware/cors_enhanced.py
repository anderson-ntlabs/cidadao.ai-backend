...
class EnhancedCORSMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_preflight(
        ...
        if requested_method and requested_method not in self.allowed_methods:
            logger.warning(
                "cors_preflight_method_denied", origin=origin, method=requested_method
            )
            return PlainTextResponse("Method not allowed", status_code=403)
        ...
        response.headers["Access-Control-Allow-Headers"] = ", ".join(
            self._get_allowed_headers(requested_headers)
        )
        response.headers["Access-Control-Allow-Credentials"] = str(
            self.allow_credentials
        ).lower()
    ...
    def _is_origin_allowed(self, origin: str) -> bool:
        ...
        logger.debug(
            "cors_origin_denied", origin=origin, allowed_count=len(self.allowed_origins)
        )
