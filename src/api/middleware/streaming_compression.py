...
class GzipStream:
    ...
    def __init__(self, level: int = 6):
        ...
        self._gzip = gzip.GzipFile(fileobj=self._buffer, mode="wb", compresslevel=level)
        ...
        async def wrapped_send(message: Message) -> None:
            ...
            if message["type"] == "http.response.start":
                ...
                if self._should_compress_stream(content_type):
                    ...
                    new_headers.extend(
                        [(b"content-encoding", b"gzip"), (b"vary", b"Accept-Encoding")]
                    )

                    message["headers"] = new_headers

                    logger.debug(
                        "streaming_compression_enabled", content_type=content_type
                    )
...
async def compress_streaming_response(
    ...
    async def compressed_iterator():
        ...
        try:
            ...
            logger.error("streaming_compression_error", error=str(e), exc_info=True)
            raise
    ...
    return StreamingResponse(compressed_iterator(), headers=headers)
