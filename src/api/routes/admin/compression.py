...
@router.get("/metrics")
async def get_compression_metrics(admin_user=Depends(require_admin)):
    """
    ...
    try:
        ...
        logger.error("compression_metrics_error", error=str(e), exc_info=True)
        raise HTTPException(
...
async def get_optimization_suggestions(admin_user=Depends(require_admin)):
    ...
    try:
        ...
        logger.error("compression_optimization_error", error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_available_algorithms(admin_user=Depends(require_admin)):
    """
...
async def test_compression(
    ...
    for accept_encoding in ["gzip", "br", "zstd", "deflate", "gzip, br"]:
        compressed, encoding, metrics = compression_service.compress(
            data=test_data, content_type="text/plain", accept_encoding=accept_encoding
        )
