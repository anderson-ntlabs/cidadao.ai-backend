...
from src.services.ip_whitelist_service import IPWhitelist, ip_whitelist_service
...
async def add_ip_to_whitelist(
    request: IPWhitelistRequest, admin_user=Depends(require_admin), db=Depends(get_db)
):
    """
    ...
    try:
        ...
        logger.error("admin_ip_whitelist_add_error", error=str(e), exc_info=True)
        raise HTTPException(
...
async def remove_ip_from_whitelist(
    ...
    removed = await ip_whitelist_service.remove_ip(
        session=db, ip_address=ip_address, environment=environment
    )
...
async def list_whitelisted_ips(
    ...
    entries = await ip_whitelist_service.list_ips(
        session=db, environment=environment, include_expired=include_expired
    )
...
async def check_ip_whitelist(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid IP address format"
        )
...
async def cleanup_expired_entries(
    ...
    count = await ip_whitelist_service.cleanup_expired(
        session=db, environment=environment
    )
    ...
    return {"status": "cleaned", "removed_count": count, "environment": environment}
...
async def initialize_default_whitelist(
    ...
    count = await ip_whitelist_service.initialize_defaults(
        session=db, created_by=admin_user.get("email", "admin")
    )
...
@router.get("/stats")
async def get_whitelist_stats(admin_user=Depends(require_admin), db=Depends(get_db)):
    """
    ...
    for env in environments:
        entries = await ip_whitelist_service.list_ips(
            session=db, environment=env, include_expired=True
        )
    ...
    return {"statistics": stats, "current_environment": settings.app_env}
