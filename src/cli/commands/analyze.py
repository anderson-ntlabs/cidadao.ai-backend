...
def display_dashboard(result: AnalysisResult):
    ...
    if result.correlations:
        ...
        for corr in result.correlations[:5]:
            ...
            color = (
                "red" if strength >= 0.9 else "yellow" if strength >= 0.7 else "green"
            )
            corr_table.add_row(
                corr.get("var1", "N/A"),
                corr.get("var2", "N/A"),
    ...
    if result.trends:
        ...
        for trend in result.trends[:3]:
            direction = (
                ...
                else "↘️" if trend.get("direction") == "down" else "→"
            )
            console.print(
...
def display_table(result: AnalysisResult):
    ...
    # Main results table
    table = Table(
        title="Analysis Results", show_header=True, header_style="bold magenta"
    )
...
def analyze(
    ...
    try:
        ...
        with Progress(
            ...
            while True:
                status_data = asyncio.run(
                    call_api(f"/api/v1/analysis/{analysis_id}", auth_token=api_key)
                )
        ...
        if status == "failed":
            console.print(
                f"[red]❌ Analysis failed: {status_data.get('error', 'Unknown error')}[/red]"
            )
            raise typer.Exit(1)
        ...
        if output == OutputFormat.DASHBOARD:
            ...
            if show_outliers and result.outliers:
                ...
                for outlier in result.outliers[:10]:
                    console.print(
                        f"  • {outlier.get('description', 'N/A')} [dim](score: {outlier.get('score', 0):.2f})[/dim]"
                    )
