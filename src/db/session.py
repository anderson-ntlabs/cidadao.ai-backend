...
async def get_session(read_only: bool = False) -> AsyncGenerator[AsyncSession, None]:
    ...
    async with connection_pool_service.get_db_session(
        pool_name="main", read_only=read_only
    ) as session:
        yield session
...
async def init_database():
    ...
    try:
        ...
        logger.info(
            "Database connection pools initialized successfully (Railway + Supabase)"
        )
    except Exception as e:
        logger.error("Failed to initialize database pools", error=str(e), exc_info=True)
        # Don't raise - allow app to start without database
        logger.warning(
            "Running without database connection - some features may be limited"
        )
...
async def close_database():
    ...
    try:
        ...
        logger.error("Failed to close database pools", error=str(e), exc_info=True)
