...
class PriorityQueueService:
    ...
    def __init__(self, max_workers: int = 5):
        ...
        logger.info("priority_queue_initialized", max_workers=max_workers)
    ...
    async def start(self):
        ...
        logger.info("priority_queue_started", workers=len(self._workers))
    ...
    def register_handler(self, task_type: str, handler: Callable):
        ...
        logger.info(
            "task_handler_registered", task_type=task_type, handler=handler.__name__
        )
        ...
        async with self._lock:
            for task in self._queue:
                ...
                tasks_by_priority[priority_name] = (
                    tasks_by_priority.get(priority_name, 0) + 1
                )
    ...
    async def _worker(self, worker_id: str):
        ...
        while self._running:
            try:
                ...
                logger.error(f"Worker {worker_id} error", error=str(e), exc_info=True)
    ...
    async def _process_task(self, task: PriorityTask, worker_id: str):
        ...
        try:
            ...
            result = await asyncio.wait_for(
                handler(task.payload, task.metadata), timeout=task.timeout
            )
    ...
    async def _handle_task_failure(
        self, task: PriorityTask, worker_id: str, error: str, start_time: datetime
    ):
    ...
    async def _execute_callback(self, task: PriorityTask, result: TaskResult):
        ...
        try:
            ...
            logger.info(
                "callback_executed", task_id=task.task_id, callback=task.callback
            )
