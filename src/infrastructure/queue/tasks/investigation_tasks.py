...
from src.services.investigation_service_selector import (
    investigation_service as InvestigationService,
)
...
async def _run_investigation_async(
    ...
    async with get_db_session() as db:
        ...
        investigation = await investigation_service.create(
            query=query, context=config or {}, initiated_by="celery_task"
        )

        # Run investigation with agents
        result = await investigation_service.run_investigation(
            investigation_id=investigation.id, agent_pool=agent_pool
        )
...
def analyze_contracts_batch(
    contract_ids: list[str], analysis_type: str = "anomaly", threshold: float = 0.7
) -> dict[str, Any]:
    """
    ...
    for contract_id in contract_ids:
        task = analyze_single_contract.s(
            contract_id=contract_id, analysis_type=analysis_type, threshold=threshold
        )
        tasks.append(task)
    ...
    summary = {
        ...
        "anomalies_found": sum(
            1 for r in contract_results if r.get("has_anomaly", False)
        ),
...
def analyze_single_contract(
    contract_id: str, analysis_type: str, threshold: float
) -> dict[str, Any]:
...
async def _analyze_contract_async(
    ...
    async with get_db_session() as db:
        ...
        analysis = await zumbi.analyze_contract(
            contract, threshold=threshold, analysis_type=analysis_type
        )
    ...
    async with get_db_session() as db:
        ...
        for item in data:
            result = await zumbi.detect_anomalies(
                data=item, config=detection_config or {}
            )
...
def emergency_investigation(
    query: str, reason: str, initiated_by: str
) -> dict[str, Any]:
    """
