...
def rotate_api_keys() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("api_key_rotation_task_failed", error=str(e), exc_info=True)
        raise
...
def cleanup_cache(older_than_hours: int = 24, pattern: str = "*") -> dict[str, Any]:
    """
    ...
    """
    logger.info(
        "cache_cleanup_started", older_than_hours=older_than_hours, pattern=pattern
    )
...
async def _cleanup_cache_async(older_than_hours: int, pattern: str) -> dict[str, Any]:
...
def optimize_database() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("database_optimization_failed", error=str(e), exc_info=True)
        raise
        ...
        for table in tables:
            try:
                ...
                optimizations.append(
                    {"table": table, "operation": "ANALYZE", "status": "success"}
                )
            except Exception as e:
                optimizations.append(
                    {
...
def warm_cache() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("cache_warming_failed", error=str(e), exc_info=True)
        raise
