...
def generate_report(
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _generate_report_async(
                    self, report_id, report_type, investigation_ids, config
                )
            )
        ...
        logger.error(
            "report_generation_failed", report_id=report_id, error=str(e), exc_info=True
        )
...
async def _generate_report_async(
    ...
    async with get_db_session() as db:
        ...
        task.update_state(
            state="PROGRESS",
            meta={"current": 40, "total": 100, "status": "Analyzing findings..."},
        )

        # Generate report content
        report_content = await tiradentes.generate_report(
            report_type=report_type, investigations=investigations, config=config or {}
        )
        ...
        task.update_state(
            state="PROGRESS",
            meta={"current": 100, "total": 100, "status": "Report completed!"},
        )
...
def generate_executive_summary(
    investigation_ids: list[str], max_length: int = 500
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _generate_executive_summary_async(investigation_ids, max_length)
            )
        ...
        logger.error("executive_summary_failed", error=str(e), exc_info=True)
        raise


async def _generate_executive_summary_async(
    investigation_ids: list[str], max_length: int
) -> dict[str, Any]:
    ...
    async with get_db_session() as db:
        ...
        summary = await tiradentes.generate_executive_summary(
            investigations=investigations, max_length=max_length
        )
...
def batch_report_generation(report_configs: list[dict[str, Any]]) -> dict[str, Any]:
    """
    ...
    """
    logger.info("batch_report_generation_started", report_count=len(report_configs))
...
def generate_urgent_report(
    investigation_id: str, reason: str, recipients: list[str]
) -> dict[str, Any]:
    """
...
def export_report_to_pdf(report_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {**report_data, "pdf_size": len(pdf_content), "pdf_generated": True}

    except Exception as e:
        logger.error(
            "pdf_export_failed", report_id=report_data.get("report_id"), error=str(e)
        )
        raise
...
def send_report_notifications(
    report_data: dict[str, Any], recipients: list[str]
) -> dict[str, Any]:
