...
class LLMService:
    ...
    async def chat(
        ...
        conversation.messages.append(LLMChatMessage(role="user", content=new_message))
    ...
    def _format_correlations_for_prompt(
        ...
        for i, corr in enumerate(correlations[:3], 1):  # Limit to top 3
            ...
            lines.append(
                f"   Coeficiente: {corr.get('correlation_coefficient', 0):.3f}"
            )
            lines.append(
                f"   Interpretação: {corr.get('business_interpretation', 'N/A')}"
            )
...
def create_llm_service(
    ...
    config = LLMServiceConfig(
        primary_provider=primary_provider, enable_fallback=enable_fallback, **kwargs
    )
