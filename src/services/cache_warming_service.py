...
class CacheWarmingConfig:
    ...
    PRIORITY_WEIGHTS = {"recency": 0.3, "frequency": 0.4, "importance": 0.3}
...
class CacheWarmingService:
    ...
    async def start_warming_scheduler(self):
        ...
        while True:
            try:
                ...
                logger.error(
                    "cache_warming_scheduler_error", error=str(e), exc_info=True
                )
    ...
    async def warm_all_caches(self):
        ...
        self._last_warming["all"] = datetime.now(UTC)
    ...
    async def _warm_popular_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            for contract_id in popular_contracts[
                : self._config.MAX_ITEMS_PER_TYPE["contracts"]
            ]:
            ...
            logger.info("popular_data_warmed", contracts_warmed=warmed_count)
    ...
    async def _warm_recent_investigations(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("recent_investigations_warmed", count=warmed_count)
    ...
    async def _warm_frequent_queries(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("frequent_queries_warmed", count=warmed_count)
    ...
    async def _warm_agent_pools(self) -> dict[str, Any]:
        ...
        try:
            ...
            agent_types = ["zumbi", "anita", "tiradentes", "machado", "dandara"]
            ...
            logger.info("agent_pools_warmed", count=warmed_count)
    ...
    async def _warm_static_resources(self) -> dict[str, Any]:
        ...
        try:
            ...
            for key, data in static_data.items():
                ...
                await cache_service.set(
                    cache_key, data, ttl=self._config.TTL_CONFIG["static_data"]
                )
                warmed_count += 1

            logger.info("static_resources_warmed", count=warmed_count)
    ...
    async def warm_specific_data(
        ...
        for identifier in identifiers:
            try:
                ...
                logger.error(f"Failed to warm {data_type}:{identifier}: {e}")
                failed.append(identifier)

        return {"warmed": warmed, "failed": failed, "total": len(identifiers)}
