...
class TestAgentContext:
    ...
    def test_context_to_dict(self):
        ...
        context = AgentContext(
            investigation_id="test-123", user_id="user-456", metadata={"key": "value"}
        )
...
class TestAgentMessage:
    ...
    def test_message_creation(self):
        ...
        message = AgentMessage(
            sender="sender_agent", recipient="recipient_agent", action="test_action"
        )
...
class TestBaseAgent:
    ...
    async def test_successful_execution(self, agent_context):
        ...
        response = await agent.execute(
            action="test_action", payload={"key": "value"}, context=agent_context
        )
    ...
    async def test_execution_with_retry(self, agent_context):
        ...
        response = await agent.execute(
            action="test_action", payload={}, context=agent_context
        )
    ...
    async def test_execution_failure_exhausted_retries(self, agent_context):
        ...
        with pytest.raises(AgentExecutionError) as exc_info:
            await agent.execute(action="test_action", payload={}, context=agent_context)
    ...
    async def test_execute_creates_proper_message(self, agent_context):
        ...
        await agent.execute(
            action="test_action", payload={"test": "data"}, context=agent_context
        )
...
class TestReflectiveAgent:
    ...
    def test_reflective_agent_initialization(self):
        ...
        agent = MockReflectiveAgent(reflection_threshold=0.8, max_reflection_loops=5)
    ...
    async def test_process_with_reflection_success(self, agent_context):
        ...
        message = AgentMessage(
            sender="test", recipient="reflective_test_agent", action="reflect_action"
        )
    ...
    async def test_process_with_reflection_max_loops(self, agent_context):
        ...
        agent = MockReflectiveAgent(
            reflection_threshold=0.95, max_reflection_loops=2  # Very high threshold
        )
...
class TestAsyncBehavior:
    ...
    async def test_concurrent_execution(self, agent_context):
        ...
        tasks = [
            agent.execute("test_action", {"id": i}, agent_context) for i in range(3)
        ]
...
class TestAgentIntegration:
    ...
    async def test_agent_message_flow(self):
        ...
        response = await receiver_agent.execute(
            action="test_action", payload={"from": sender_agent.name}, context=context
        )
