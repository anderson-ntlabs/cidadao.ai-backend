...
from src.api.routes.export import (
    _format_investigation_as_markdown,
    bulk_export,
    export_anomalies,
    export_contracts,
...
class TestExportRoutes:
    ...
    async def test_export_investigation_excel(
        ...
        response = await export_investigation(
            investigation_id="INV-001", format="excel", current_user=mock_current_user
        )
    ...
    async def test_export_investigation_csv(
        ...
        # Call function
        response = await export_investigation(
            investigation_id="INV-001", format="csv", current_user=mock_current_user
        )
    ...
    async def test_export_investigation_pdf(
        ...
        # Call function
        response = await export_investigation(
            investigation_id="INV-001", format="pdf", current_user=mock_current_user
        )
    ...
    async def test_export_contracts_not_found(
        ...
        # Create request
        request = ExportRequest(
            export_type="contracts", format="excel", filters={"year": 2025}
        )
    ...
    async def test_export_anomalies_none_found(
        ...
        # Create request
        request = ExportRequest(export_type="anomalies", format="excel")
    ...
    def test_export_request_validation(self):
        ...
        # Valid request
        request = ExportRequest(export_type="investigations", format="excel")
        ...
        with pytest.raises(ValueError) as exc_info:
            ExportRequest(export_type="invalid_type", format="excel")
        assert "Export type must be one of" in str(exc_info.value)

        # Invalid format
        with pytest.raises(ValueError) as exc_info:
            ExportRequest(export_type="contracts", format="invalid_format")
    ...
    def test_bulk_export_request_validation(self):
        ...
        # Valid request
        request = BulkExportRequest(exports=[{"type": "investigation", "id": "123"}])
