...
"""
    ...
    async def test_generate_excel_multiple_sheets(self, service, sample_dataframe):
        ...
        excel_bytes = await service.generate_excel(
            data=data, title="Complete Report", metadata={"version": "1.0"}
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_bulk_export(
        self, service, sample_markdown_content, sample_dataframe
    ):
    ...
    async def test_convert_investigation_to_excel(self, service):
        ...
        investigation_data = {
            ...
            "contracts": [
                {"id": "C001", "value": 500000, "supplier": "Company A"},
                {"id": "C002", "value": 300000, "supplier": "Company B"},
            ],
            "results": {
                "total_analyzed": 100,
                "anomalies_found": 2,
    ...
    async def test_large_content_handling(self, service):
        ...
        pdf_bytes = await service.generate_pdf(
            content=large_content, title="Large Report", format_type="report"
        )
...
"""
        ...
        pdf_bytes = await service.generate_pdf(
            content=content_with_special, title="Relatório Especial"
        )
